...
function ProgressDetails({ participantId, refreshTrigger }: { participantId: number; refreshTrigger?: number }) {
        ...
        <div key={entry.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <div className="flex justify-between items-start mb-2">
            <div className="font-medium text-gray-900">
              {entry.units_completed} units completed
            </div>
...
export default function Participant() {
  ...
  const [progressForm, setProgressForm] = useState(() => {
    try {
      ...
      console.error('Error initializing progress form:', error);
      return {
        units_completed: '',
        log_date: '2025-01-01', // Fallback date
        notes: '',
        selected_dates: [] as string[],
        image: null as File | null
      };
  ...
  const fetchParticipant = useCallback(async () => {
    try {
      console.log('Fetching participant data...', { id, timestamp: Date.now() });
      setLoading(true);
  ...
  useEffect(() => {
    if (id) {
      ...
      // Handle action=progress parameter from push notifications
      if (searchParams.get('action') === 'progress' && user?.id === participant?.user_id) {
        try {
          ...
          console.error('Error opening progress form from notification:', error);
        }
      }
    }
  ...
  const handlePledgeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          const pledgeTypeText = pledgeForm.pledge_type === 'flat_rate' 
            ? `$${pledgeForm.flat_amount} donation`
            : pledgeForm.pledge_type === 'per_unit_capped'
            ? `$${pledgeForm.amount_per_unit} per ${(participant?.unit || 'unit').slice(0, -1)} (up to $${pledgeForm.max_total_amount})`
            : `$${pledgeForm.amount_per_unit} per ${(participant?.unit || 'unit').slice(0, -1)}`;
  ...
  const handleProgressSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (progressForm.image) {
        ...
        formData.append('participant_id', participant.id.toString());

        try {
          const uploadResponse = await fetch('/wapi/upload-image', {
            method: 'POST',
            body: formData,
          });
      ...
      if (response.ok) {
        ...
        if (window.confirm(`Progress logged successfully! You completed ${unitsCompleted} ${participant.unit}.\n\nWould you like to share your progress update with supporters?`)) {
  ...
  const progressPercentage = participant?.current_progress && participant?.goal_amount 
  ...
  const handleCalendarReminder = () => {
    ...
    const reminderEvent = createChallengeEndReminder(
      'This participant',
            ...
            <div className="text-center">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6 mb-8">
                <div className="grid md:grid-cols-3 gap-6 text-center">
                  ...
                  <div>
                    <div className="text-3xl font-bold">${(Number(participant?.total_raised) || 0).toFixed(0)}</div>
              ...
              <div className="space-y-4 sm:space-y-0 sm:grid sm:grid-cols-1 md:grid-cols-2 sm:gap-6 max-w-4xl mx-auto">
                <div className="bg-white rounded-xl p-4 sm:p-6 border border-gray-200">
                  <div className="text-center mb-4">
                    ...
                    <h3 className="text-xl sm:text-2xl font-bold mb-2 text-black">Pledge Per {(participant?.unit || 'unit').slice(0, -1)}</h3>
                    <p className="text-gray-600 text-sm">Support them for every {(participant?.unit || 'unit').slice(0, -1)} they complete!</p>
                  </div>
                  <button
                  ...
                  <p className="text-gray-500 text-xs mt-3 text-center">
                    Choose your amount per {(participant?.unit || 'unit').slice(0, -1)} • Set a maximum if you want
                  </p>
                </div>
                ...
                <div className="bg-white rounded-xl p-4 sm:p-6 border border-gray-200">
                  ...
                  <button
                    onClick={() => {
                      const donationUrl = 'https://www.every.org/brain-fog-recovery-source?donateTo=brain-fog-recovery-source#/donate/card';
                  ...
                  <p className="text-gray-500 text-xs mt-3 text-center">
                    Direct donation via Every.org • Tax-deductible
                  </p>
                </div>
              </div>
              ...
              <div className="mt-8 bg-gray-50 rounded-lg p-4 border border-gray-200">
                <p className="text-gray-800 text-sm">
                  <strong>How Pledging Works:</strong> Choose an amount per {(participant?.unit || 'unit').slice(0, -1)} completed. 
                  You'll be notified when the challenge ends to fulfill your pledge. All funds support psychiatric recovery access through Brain Fog Recovery Source.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-8 text-gray-900 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-gray-600">Goal: {participant.goal_amount} {participant.unit}</p>
              ...
              <div className="flex items-center justify-between">
                <div className="flex space-x-6">
                  ...
                  <div>
                    <div className="text-2xl font-bold">${(Number(participant?.total_raised) || 0).toFixed(2)}</div>
                    <div className="text-gray-700 text-sm">Raised So Far</div>
                    <div className="text-gray-600 text-xs">${(Number(participant?.total_potential) || 0).toFixed(2)} potential</div>
                  </div>
                </div>
                {!isOwner && (
                  <div className="text-center">
                    ...
                    <p className="text-gray-600 text-sm mt-2">Pledge per unit completed or make a one-time donation</p>
                  </div>
                )}
                {isOwner && (
                    ...
                    <button
                      onClick={() => {
                        try {
                          ...
                          console.error('Error opening progress form:', error);
                          alert('Unable to open progress form. Please refresh the page and try again.');
                        }
                      }}
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md my-8 max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-blue-50 p-3 rounded-lg mb-4">
              <p className="text-sm text-blue-700">
                <strong>Choose your pledge type:</strong> Support this challenge with flexible donation options that work for your budget.
              </p>
            </div>
                ...
                <div className="space-y-3">
                  <label className="flex items-start p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                    <input
                      ...
                      onChange={(e) => setPledgeForm({...pledgeForm, pledge_type: e.target.value as 'per_unit_uncapped' | 'per_unit_capped' | 'flat_rate'})}
                      className="mt-1 mr-3"
                    />
                    <div className="flex-1">
                      <div className="font-medium text-gray-900">Per {(participant?.unit || 'unit').slice(0, -1)} (unlimited)</div>
                      <div className="text-sm text-gray-600">Pay for each {(participant?.unit || 'unit').slice(0, -1)} completed</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                    ...
                    />
                    <div className="flex-1">
                      <div className="font-medium text-gray-900">Per {(participant?.unit || 'unit').slice(0, -1)} (capped)</div>
                      <div className="text-sm text-gray-600">Pay per {(participant?.unit || 'unit').slice(0, -1)} up to a maximum total</div>
                    </div>
                  </label>
              ...
              {(pledgeForm.pledge_type === 'per_unit_uncapped' || pledgeForm.pledge_type === 'per_unit_capped') && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Amount per {participant.unit.slice(0, -1)} completed
                  </label>
                  ...
                  {pledgeForm.pledge_type === 'per_unit_uncapped' && pledgeForm.amount_per_unit && participant && (
                    <p className="text-xs text-gray-600 mt-1">
                      Maximum possible: ${((parseFloat(pledgeForm.amount_per_unit) || 0) * (participant?.goal_amount || 0)).toFixed(2)} if they reach their full goal
                    </p>
                  )}
                </div>
              )}
              ...
              {pledgeForm.pledge_type === 'per_unit_capped' && (
                <div>
                  ...
                  {pledgeForm.amount_per_unit && pledgeForm.max_total_amount && participant && (
                    <p className="text-xs text-gray-600 mt-1">
                      You'll pay ${pledgeForm.amount_per_unit} per {(participant?.unit || 'unit').slice(0, -1)} up to {Math.floor(parseFloat(pledgeForm.max_total_amount) / parseFloat(pledgeForm.amount_per_unit))} {participant?.unit || 'units'}
                    </p>
                  )}
                </div>
              )}
              ...
              {pledgeForm.pledge_type === 'flat_rate' && (
                <div>
                  ...
                  <p className="text-xs text-gray-600 mb-2">
                    You'll pay this amount regardless of how much they complete
                  </p>
                  <div className="relative">
                    <span className="absolute left-3 top-2 text-gray-500">$</span>
                    <input
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md my-8 max-h-[90vh] overflow-y-auto">
              ...
              <button
                ...
                onClick={() => {
                  try {
                    ...
                    console.error('Error closing progress form:', error);
                    setShowProgressForm(false);
                  }
                }}
                className="p-1 text-gray-400 hover:text-gray-600 transition-colors"
            ...
            <div className="bg-blue-50 p-3 rounded-lg mb-4">
              <p className="text-sm text-blue-700 font-medium">
                Challenge: {participant?.challenge_name || 'N/A'}
              </p>
            ...
            <form onSubmit={handleProgressSubmit} className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-600 mb-2">
                  Select any date you want - we don't restrict when you can log progress
                </p>
                <input
                  type="date"
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  For: {participant?.challenge_name || 'Challenge'}
                </p>
              </div>
              <div>
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button
                  ...
                  disabled={
                    progressLoading || 
                    !progressForm.units_completed || 
                    !progressForm.log_date
                  }
                  className="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors"
                >
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md my-8 max-h-[90vh] overflow-y-auto">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600 text-sm leading-relaxed mb-4">
                Your support means a lot. Would you like a calendar reminder for when this challenge ends so you'll know when to complete your donation?
              </p>
              <div className="bg-green-50 p-3 rounded-lg mb-4">
                <p className="text-sm text-green-700">
                  <strong>We'll create a reminder</strong> for 30 days from now with a link back to this campaign and your donation details.
                </p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-xl p-6 w-full max-w-md my-8 max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to end this fundraising campaign? This will mark it as completed and allow you to start a new campaign in the future. This action cannot be undone.
            </p>
            <div className="flex space-x-3">
              <button
                type="button"
