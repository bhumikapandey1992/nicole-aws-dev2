...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch spotlight (defensive to handle 404/empty)
      try {
        const sRes = await fetch('/wapi/spotlight', { headers: { 'cache-control': 'no-cache' }});
        if (sRes.ok) {
          const sData = await sRes.json();
          // Accept either an object or {spotlight: {...}}
          const s: Spotlight | null = sData?.spotlight ?? sData ?? null;
          if (s && typeof s === 'object' && s.id) setSpotlight(s);
        }
      } catch {
        console.debug('No spotlight available yet.');
      }

      // NOTE: Previously this page auto-redirected to the most recent participant.
      // We now keep users on the dashboard so they can see Spotlight + actions.
      // If you want to re-enable, fetch /wapi/my-participants and redirect here.
    } catch (error) {
          ...
          {spotlight && (
              <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm mb-8">
                ...
                {spotlight.spotlight_image_url ? (
                    <div className="w-full h-64 md:h-80 overflow-hidden bg-gray-50">
                      <img
